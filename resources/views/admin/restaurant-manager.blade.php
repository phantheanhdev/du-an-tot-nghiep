...
})->count() > 0 ? route('order-of-table', $table->id) : '#' }}" class="text-white">


                        <div id="table-{{$table->id}}" class="widget p-lg text-center {{ $table->orders->filter(function ( $order) {
    return $order->status != 2 && $order->status != 5;
})->count() > 0 ? 'green-bg' : 'black-bg' }} " style="height: 160px;">
